  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--ios-bg)",paddingBottom:83}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"16px 14px 40px"}}>
        ...
        {/* ── PRÓXIMAS ── */}
        {view==="upcoming" && (()=>{
          const now7 = new Date(); const end7 = new Date(); end7.setDate(now7.getDate()+7);
          const next7 = [...apts].filter(a=>{const d=new Date(a.date+"T12:00");return d>=now7&&d<=end7;}).sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time));
          return (
          <div className="fu">
            <div style={{marginBottom:14}}>
              <h2 style={{fontSize:22,fontWeight:700,color:"var(--ios-label)",letterSpacing:"-.4px",marginBottom:2}}>Próximas 7 días</h2>
              <p style={{fontSize:13,color:"var(--ios-label2)"}}>{next7.length} cita{next7.length!==1?"s":""} esta semana</p>
            </div>
            <AddAppointmentForm db={db} services={services} onSave={()=>showToast("✅ Cita guardada")}/>
            {next7.length===0
              ? <div className="card" style={{padding:"32px",textAlign:"center",color:"var(--ios-label2)",fontSize:15}}>Sin citas en los próximos 7 días</div>
              : next7.map(apt=><AptCard key={apt._id} apt={apt} db={db} services={services} onDel={()=>del(apt._id)} onConf={()=>conf(apt._id,apt.confirmed)} onWa={()=>waMsg(apt)} onToast={showToast}/>)
            }
          </div>
        );})()}
...
function CitasManager({ apts, db, services, cfg, onToast }) {
  ...
  const toggleConf = async (id, cur, apt) => {
    ...
    if (!cur) {
      // Notify client via WhatsApp when confirming
      const salonName = cfg?.salonName || "Nail Studio";
      const m = `¡Hola ${apt.clientName}! 👋💅\n\nTu cita ha sido *confirmada* en ${salonName}:\n📅 ${fmtLong(apt.date)}\n⏰ ${apt.time}\n${apt.serviceEmoji||"💎"} ${apt.service}\n💰 $${apt.price}\n\n¡Te esperamos con mucho cariño! ✨`;
      window.open(`https://wa.me/${cleanPhone(apt.phone||"")}?text=${encodeURIComponent(m)}`, "_blank");
    ...
    <div className="fu">
      ...
      {filter==="pendientes" && pending>0 && (
        <div style={{background:"rgba(37,211,102,.08)",border:"1px solid rgba(37,211,102,.2)",borderRadius:12,padding:"10px 14px",marginBottom:14,fontSize:12,color:"#1A7A3A",display:"flex",alignItems:"center",gap:8}}>
          <MessageCircle size={14} color="#1A7A3A"/>
          Al confirmar una cita, se abrirá WhatsApp para notificar a la clienta automáticamente.
        </div>
      )}
              ...
              <div key={apt._id} className="card" style={{padding:"16px",borderRadius:16,border:apt.confirmed?"1.5px solid rgba(52,199,89,.25)":apt.selfBooked&&!apt.confirmed?"1.5px solid rgba(255,149,0,.3)":"1.5px solid transparent"}}>
                {isEditing ? (
                  ...
                  <>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:14}}>
                      ...
                      <button
                        ...
                        style={{borderRadius:10,padding:"9px 6px",fontSize:12,fontWeight:600,cursor:"pointer",border:"none",
                          background:apt.confirmed?"rgba(255,149,0,.12)":"rgba(37,211,102,.13)",
                          color:apt.confirmed?"#A06000":"#1A7A3A",
                          display:"flex",alignItems:"center",justifyContent:"center",gap:4,transition:"all .15s",fontFamily:"-apple-system,sans-serif"}}>
                        {apt.confirmed ? <><X size={12}/>Quitar conf.</> : <><Check size={12}/>Confirmar</>}
                      </button>
                    ...
                    {/* WhatsApp confirm button (only if confirmed and has phone) */}
                    {apt.confirmed && apt.phone && (
                      <button
                        onClick={()=>{
                          const salonName = cfg?.salonName||"Nail Studio";
                          const m = `¡Hola ${apt.clientName}! 👋💅\n\nTe recordamos tu cita en ${salonName}:\n📅 ${fmtLong(apt.date)}\n⏰ ${apt.time}\n${apt.serviceEmoji||"💎"} ${apt.service}\n💰 $${apt.price}\n\n¡Te esperamos! ✨`;
                          window.open(`https://wa.me/${cleanPhone(apt.phone)}?text=${encodeURIComponent(m)}`,"_blank");
                        }}
...
function ClientsManager({ db, clients, apts, onToast }) {
  ...
  const delClient = async (id) => {
    ...
    onToast("Clienta eliminada");
  };
  const confApt = async (id, cur) => {
    await updateDoc(doc(db,"appointments",id),{confirmed:!cur});
    onToast(cur ? "Cita marcada pendiente" : "✅ Cita confirmada");
  };
  ...
  if(clients.length===0) return (
    <div className="card" style={{padding:"40px",textAlign:"center",color:"var(--ios-label2)",fontSize:15}}>
      Cuando alguien agende aparecerá aquí
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="ios-list">
        {filtered.map((c,idx)=>{
          const clientApts = apts.filter(a=>cleanPhone(a.phone||"")===cleanPhone(c.phone||"")).sort((a,b)=>b.date.localeCompare(a.date));
          const cnt   = clientApts.length;
          const total = clientApts.reduce((s,a)=>s+(a.price||0),0);
          ...
          return (
            <div key={c._id} style={{borderBottom:idx<filtered.length-1?".5px solid var(--ios-sep)":"none"}}>
              {isEditing ? (
                ...
                <>
                  {/* Row */}
                  <div className="ios-row" onClick={()=>{playTap();setExpandedId(isExpanded?null:c._id);}}>
                    {/* Initial circle */}
                    <div style={{width:40,height:40,borderRadius:"50%",background:`${avatarColor}20`,border:`2px solid ${avatarColor}40`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:16,fontWeight:700,color:avatarColor}}>
                      {initial}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:16,fontWeight:500,color:"var(--ios-label)",letterSpacing:"-.2px"}}>{c.name}</div>
                      <div style={{fontSize:13,color:"var(--ios-label2)"}}>{c.phone}</div>
                    </div>
                    <div style={{textAlign:"right",flexShrink:0}}>
                      <div style={{fontSize:14,fontWeight:600,color:"var(--ios-tint)"}}>{cnt} cita{cnt!==1?"s":""}</div>
                      <div style={{fontSize:12,color:"var(--ios-label2)"}}>${total}</div>
                    </div>
                    <span style={{marginLeft:4,fontSize:13,color:"var(--ios-label3)",transition:"transform .2s",display:"inline-block",transform:isExpanded?"rotate(90deg)":""}}>›</span>
                  </div>
                  {/* Expanded detail */}
                  {isExpanded && (
                    <div style={{background:"rgba(0,0,0,.025)",padding:"12px 16px 16px"}}>
                      {/* Edit/delete client buttons */}
                      <div style={{display:"flex",gap:8,marginBottom:12}}>
                        <button onClick={()=>{startEdit(c);setExpandedId(null);}} style={{flex:1,background:"rgba(175,82,222,.1)",border:"none",borderRadius:10,padding:"8px",cursor:"pointer",fontSize:13,color:"var(--ios-purple)",fontWeight:500,fontFamily:"-apple-system,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><Pencil size={12}/>Editar</button>
                        <button onClick={()=>delClient(c._id)} style={{flex:1,background:"rgba(255,59,48,.1)",border:"none",borderRadius:10,padding:"8px",cursor:"pointer",fontSize:13,color:"#FF3B30",fontWeight:500,fontFamily:"-apple-system,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><Trash2 size={12}/>Eliminar</button>
                      </div>
                      {/* Appointment list */}
                      {clientApts.length===0
                        ? <p style={{fontSize:13,color:"var(--ios-label2)",textAlign:"center",padding:"8px 0"}}>Sin citas registradas</p>
                        : clientApts.map(apt=>(
                            <div key={apt._id} style={{background:"var(--ios-card)",borderRadius:12,padding:"12px 14px",marginBottom:8,boxShadow:"0 1px 6px rgba(0,0,0,.06)"}}>
                              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                                <div>
                                  <div style={{fontSize:14,fontWeight:600,color:"var(--ios-label)",letterSpacing:"-.1px"}}>{apt.service}</div>
                                  <div style={{fontSize:12,color:"var(--ios-label2)",marginTop:1}}>{fmtShort(apt.date)} · {apt.time} · <span style={{color:"var(--ios-tint)",fontWeight:600}}>${apt.price}</span></div>
                                </div>
                                <span className="badge" style={{background:apt.confirmed?"rgba(52,199,89,.15)":"rgba(255,149,0,.15)",color:apt.confirmed?"#1A8040":"#A06000",fontSize:11}}>
                                  {apt.confirmed?"✓ Conf.":"Pendiente"}
                                </span>
                              </div>
                              <div style={{display:"flex",gap:6}}>
                                <button onClick={()=>confApt(apt._id,apt.confirmed)} style={{flex:1,background:apt.confirmed?"rgba(255,149,0,.1)":"rgba(52,199,89,.1)",border:"none",borderRadius:8,padding:"6px",cursor:"pointer",fontSize:12,color:apt.confirmed?"#A06000":"#1A8040",fontFamily:"-apple-system,sans-serif",fontWeight:500}}>
                                  {apt.confirmed?"Quitar conf.":"Confirmar"}
                                </button>
                                <button onClick={()=>delApt(apt._id)} style={{flex:1,background:"rgba(255,59,48,.08)",border:"none",borderRadius:8,padding:"6px",cursor:"pointer",fontSize:12,color:"#FF3B30",fontFamily:"-apple-system,sans-serif",fontWeight:500}}>
                                  Eliminar
                                </button>
                              </div>
                            </div>
...
function ConfigPanel({ cfg, onSave, onToast }) {
  const [f, setF] = useState(cfg || {salonName:"",salonLocation:"",profName:"",profPhoto:"",slotInterval:30,heroTitle:"",heroSubtitle:"",heroBg:""});
  useEffect(()=>{ if(cfg) setF({slotInterval:30,heroTitle:"",heroSubtitle:"",heroBg:"",profName:"",profPhoto:"",...cfg}); },[cfg]);
