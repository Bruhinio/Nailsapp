      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"16px 16px 120px",width:"100%"}}>
        ...
        <div style={{marginBottom:14}}>
          <div style={{display:"flex",gap:4,marginBottom:5}}>
            {[1,2,3,4,5,6].map(i=>(
              <div key={i} style={{flex:1,height:3,borderRadius:2,background:i<=1?"var(--ios-tint)":"rgba(60,60,67,.15)"}}/>
            ))}
          </div>
          <div style={{fontSize:11,color:"var(--ios-label2)",fontWeight:500}}>Paso 1 de 6</div>
        </div>
    ...
    <div className="page">
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"12px 16px 120px",width:"100%"}}>
        ...
        <div style={{background:"var(--ios-card)",borderRadius:14,boxShadow:"var(--ios-sh)",overflow:"hidden"}}>
          <div style={{background:"linear-gradient(90deg,var(--ios-tint),#C4184A)",padding:"10px 14px"}}>
...
function NotesPage({ booking, db, cfg, onDone, onBack }) {
    ...
    <div className="page slideUp" style={{paddingBottom:32}}>
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"16px 16px 0",width:"100%"}}>
        ...
        <div className="card fu" style={{overflow:"hidden",borderRadius:20,boxShadow:"var(--ios-sh-lg)",marginBottom:12}}>
          <div style={{background:"linear-gradient(90deg,var(--ios-tint),#C4184A)",padding:"14px 18px",display:"flex",alignItems:"center",gap:12}}>
            <span style={{fontSize:28}}>💅</span>
            <div>
              <div style={{fontSize:16,fontWeight:700,color:"#fff"}}>{cfg?.salonName||"Nail Studio"}</div>
          ...
          <div style={{padding:"14px",background:"#ECE5DD"}}>
            <div style={{background:"#DCF8C6",borderRadius:"12px 12px 12px 2px",padding:"12px 14px",maxWidth:"90%",boxShadow:"0 1px 3px rgba(0,0,0,.12)"}}>
              <div style={{fontSize:13,color:"#111",lineHeight:1.6}}>
                💅 <strong>{cfg?.salonName||"Nail Studio"}</strong><br/><br/>
    ...
    <div className="page">
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"12px 16px 120px",width:"100%"}}>
        ...
        <div className="card fu" style={{overflow:"hidden",borderRadius:20,marginBottom:12}}>
          <div style={{background:"linear-gradient(90deg,var(--ios-tint),#C4184A)",padding:"12px 18px"}}>
            <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,.8)",letterSpacing:".4px",textTransform:"uppercase"}}>Resumen de tu cita</div>
          </div>
          <div style={{padding:"16px 18px"}}>
            ...
            <div style={{background:"rgba(255,45,85,.06)",borderRadius:12,padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
  ...
  const confirm = async () => {
    ...
    try {
      ...
      // Auto-block slots covered by appointment duration
      {
        const interval = cfg?.slotInterval || 30;
        const wsH = parseInt((cfg?.workStart||"09:00").split(":")[0]);
        const wsM = parseInt((cfg?.workStart||"09:00").split(":")[1]||"0");
        ...
        if (startIdx !== -1) {
          // Primary service duration in minutes
          const primaryDur = booking.mySvc.duration || interval;
          // Companion service duration (runs AFTER primary = sequential)
          const companionDur = (booking.plus && booking.cmpSvc) ? (booking.cmpSvc.duration || interval) : 0;
          // Total minutes to block from start slot
          const totalDur = primaryDur + companionDur;
          // How many slots total (round up to next full slot)
          const totalSlots = Math.ceil(totalDur / interval);

          // Fetch existing blocked once
          const existingBlockedSnap = await getDocs(collection(db,"blocked"));
          const existingBlockedData = existingBlockedSnap.docs.map(d => d.data());

          // Block ALL slots covered (including the start slot itself as "taken by appointment"
          // — slots after start are blocked so no other client can book them)
          for (let i = 1; i < totalSlots; i++) {
            const slotIdx = startIdx + i;
            if (slotIdx >= allSlots.length) break;
            const t = allSlots[slotIdx];
            const alreadyBlocked = existingBlockedData.find(b =>
              b.type==="slot" && b.date===booking.date && b.time===t
            );
            if (!alreadyBlocked) {
              await addDoc(collection(db,"blocked"), {
                ...
                date: booking.date,
                time: t,
                reason: `Reservado: ${booking.myName}`,
                auto: true,
                aptPhone: booking.myPhone,
              });
            }
          }
        }
...
function ClientPortal({ db, services, cfg, blocked, onAdmin }) {
  ...
  if (page === "service")   return <ServicePage   booking={booking} services={services} onBack={()=>setPage("calendar")} onNext={d=>{setBk(d);setPage("companion");}}/>;
  if (page === "companion") return <CompanionPage booking={booking} services={services} onBack={()=>setPage("service")}  onNext={d=>{setBk(d);setPage("names");}}/>;
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--ios-bg)",paddingBottom:83}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"16px 14px 40px"}}>
        ...
        {/* ── CITAS ── */}
        {view==="citas" && (
          <CitasManager apts={apts} db={db} services={services} cfg={cfg} blocked={blocked} onToast={showToast}/>
        )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
        ...
        <button className="card" onClick={()=>setFilter("en_progreso")}
          style={{padding:"14px",borderRadius:16,textAlign:"center",border:"none",cursor:"pointer",
            background:inProgCnt>0?"linear-gradient(135deg,rgba(255,45,85,.12),rgba(255,45,85,.06))":"var(--ios-card)",
                  ...
                  <>
                    ...
                    {isInProgress(apt) ? (
                      ...
                      <div style={{marginTop:14}}>
                        <div style={{background:"rgba(255,45,85,.07)",borderRadius:12,padding:"12px 14px",marginBottom:10,fontSize:12,color:"var(--ios-tint)",fontWeight:500,textAlign:"center"}}>
