...
const THEMES = {
  rosa: {
    ...
    primaryLight18: "rgba(255,45,85,.18)",
    primaryLight40: "rgba(255,45,85,.4)",
    primaryLight45: "rgba(255,45,85,.45)",
  ...
  azul: {
    ...
    primaryLight18: "rgba(0,122,255,.18)",
    primaryLight40: "rgba(0,122,255,.4)",
    primaryLight45: "rgba(0,122,255,.45)",
  ...
  negro: {
    ...
    primaryLight18: "rgba(28,28,30,.18)",
    primaryLight40: "rgba(28,28,30,.4)",
    primaryLight45: "rgba(28,28,30,.45)",
  ...
  blanco: {
    ...
    primaryLight18: "rgba(90,200,250,.18)",
    primaryLight40: "rgba(90,200,250,.4)",
    primaryLight45: "rgba(90,200,250,.45)",
...
const G = ({ theme = "rosa" }) => {
  ...
  return (
    ...
    *{font-family:-apple-system,'SF Pro Text','SF Pro Display',BlinkMacSystemFont,'Helvetica Neue',sans-serif}
    ...
    body.dark .btn-o{background:${currentTheme.primaryLight18}!important;color:${currentTheme.primary}!important}
...
function ClientPortal({ db, services, cfg, blocked, onAdmin }) {
  ...
  // Construcción de estilos fuera del JSX
  const heroBg = cfg?.heroBg 
    ? `linear-gradient(rgba(0,0,0,.52),rgba(0,0,0,.52)), url(${cfg.heroBg}) center/cover no-repeat`
    : currentTheme.gradient;
  const logoGradient = `linear-gradient(135deg,${currentTheme.primary},${currentTheme.primaryDark})`;
  const logoShadow = `0 8px 24px ${currentTheme.primaryLight40}`;
  const btnShadow = `0 4px 20px ${currentTheme.primaryLight45}`;
  const promoGradient = `linear-gradient(135deg, ${currentTheme.primaryLight}, rgba(255,255,255,.5))`;

  return (
    <div className="page" style={{paddingBottom:83}}>
      <G theme={cfg?.theme} />
      {/* iOS Hero */}
      <div style={{background:heroBg,padding:"60px 20px 40px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",display:"inline-block",marginBottom:18}}>
          <button onClick={tapLogo}
            style={{background:logoGradient,border:"none",borderRadius:22,width:72,height:72,display:"flex",alignItems:"center",justifyContent:"center",cursor:"default",backdropFilter:"blur(8px)",boxShadow:logoShadow}}>
            {cfg?.logoImage ? (
        ...
        <button className="fu d3" onClick={()=>{playTap();setPage("service");}}
          style={{background:"var(--ios-tint)",color:"#fff",border:"none",borderRadius:16,padding:"16px 38px",fontSize:17,fontWeight:600,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:10,boxShadow:btnShadow,transition:"all .22s",position:"relative",zIndex:1}}
        ...
        <div style={{padding:"0 16px 24px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {cfg.promotions.map((promo,i)=>(
              <div key={i} className="card fu d1" style={{padding:"16px",background:promoGradient}}>
                {promo.image && (
  ...
  return (
    <div className="card" style={{padding:"18px"}}>
      ...
      <div style={{display:"flex",gap:14,marginTop:14,flexWrap:"wrap",justifyContent:"center"}}>
        ...
        ].map(({bg,c,l})=>{const borderStyle=`1.5px solid ${c}`;return (
          <div key={l} style={{display:"flex",alignItems:"center",gap:5}}>
            <span style={{width:10,height:10,borderRadius:"50%",background:bg,border:borderStyle}}/>
...
function DashboardTab({ apts, cfg }) {
  ...
  const getStatusBorder = (status) => {
    if (status === "confirmed") return "3px solid var(--ios-green)";
    if (status === "cancelled") return "3px solid #FF3B30";
    return "3px solid var(--ios-orange)";
  };

  const getStatusColor = (status) => {
    if (status === "confirmed") return "var(--ios-green)";
      ...
      <div className="card fu d1" style={{padding:"18px",marginTop:16}}>
        ...
        {todayApts.length===0 ? (
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {todayApts.map(a=>(
              <div key={a._id} style={{background:"var(--ios-bg)",borderRadius:12,padding:"12px",borderLeft:getStatusBorder(a.status)}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
                  <span style={{fontSize:15,fontWeight:600,color:"var(--ios-label)"}}>{a.time}</span>
                  <span style={{fontSize:11,fontWeight:600,color:getStatusColor(a.status),textTransform:"uppercase"}}>
...
function ConfigPanel({ cfg, onSave, onToast, onLogout, newApts, todayCount }) {
  ...
  const currentThemeConfig = THEMES[f.theme||"rosa"];
  const headerGradient = `linear-gradient(135deg,var(--ios-tint),${currentThemeConfig.primaryDark})`;

  return (
    <div className="fu">

      {/* ── Perfil header con logout ── */}
      <div style={{background:headerGradient,borderRadius:20,padding:"20px",marginBottom:20,position:"relative",overflow:"hidden"}}>
      ...
      <div className="ios-list" style={{marginBottom:20}}>
        <div style={{padding:"14px"}}>
          ...
          {f.logoImage && (() => {
            const logoBg = `url(${f.logoImage}) center/cover`;
            return (
              <div style={{borderRadius:14,overflow:"hidden",width:72,height:72,background:logoBg,
            ...
            {(!f.promotions || f.promotions.length === 0) ? (
              ...
              f.promotions.map((promo,i)=>(
                <div key={i} style={{padding:"14px",borderBottom:i<f.promotions.length-1?".5px solid var(--ios-sep)":"none"}}>
                  ...
                  {promo.image && (() => {
                    const promoBg = `url(${promo.image}) center/cover`;
                    return <div style={{width:"100%",height:100,borderRadius:10,overflow:"hidden",background:promoBg}}/>;
                  })()}
                </div>
              ))
            )}
        ...
        <div className="pop" style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div className="card" style={{width:"100%",maxWidth:400,padding:"24px",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{marginBottom:20}}>
              ...
              {newPromo.image && (() => {
                const newPromoBg = `url(${newPromo.image}) center/cover`;
                return (
                  <div style={{borderRadius:12,overflow:"hidden",height:140,background:newPromoBg,marginBottom:10,position:"relative"}}>
      ...
      <div className="ios-list" style={{marginBottom:20}}>
        ...
        <div style={{padding:"14px"}}>
          ...
          {f.heroBg && (() => {
            const previewBg = `linear-gradient(rgba(0,0,0,.4),rgba(0,0,0,.4)), url(${f.heroBg}) center/cover`;
            return (
              <div style={{borderRadius:14,overflow:"hidden",height:110,background:previewBg,
