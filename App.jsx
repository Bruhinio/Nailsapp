...
const G = () => (
  <style>{`
    ...
    *{font-family:'DM Sans',sans-serif} h1,h2,h3,h4{font-family:'Cormorant Garamond',serif}
    ...
    /* ─ Dark mode ─ */
    /* ═══════════════════════════════════════════════
       DARK MODE — Full contrast system
       ═══════════════════════════════════════════════ */
    body.dark{background:#0F0B10!important}
    body.dark *{color:#F0E8F4}
    body.dark .page{background:#0F0B10}

    /* Cards & surfaces */
    body.dark .card{background:#1E1520;border:1px solid #3A2840;box-shadow:0 4px 24px rgba(0,0,0,.4)}
    body.dark .topbar{background:#1E1520;border-color:#3A2840}

    /* Text hierarchy */
    body.dark h1,body.dark h2,body.dark h3,body.dark h4{color:#FAF0F6!important}
    body.dark p,body.dark span,body.dark div,body.dark label{color:#DDD0E4}
    body.dark .text2,body.dark [style*="var(--text2)"]{color:#A090B0!important}

    /* Inputs */
    body.dark input,body.dark select,body.dark textarea{background:#2A1E30;border-color:#4A3458;color:#FAF0F6!important}
    body.dark input::placeholder,body.dark textarea::placeholder{color:#7A6080}
    body.dark input:focus,body.dark select:focus,body.dark textarea:focus{border-color:#C4627E;box-shadow:0 0 0 3px rgba(196,98,126,.25)}

    /* Buttons */
    body.dark .btn-o{border-color:#7A4468;color:#F0A0C0;background:transparent}
    body.dark .btn-o:hover{background:#3A1A28}
    body.dark .btn-ghost{color:#C090B0}
    body.dark .btn-ghost:hover{color:#F0A0C0}

    /* Navigation tabs */
    body.dark .nav-tab{color:#9080A0}
    body.dark .nav-tab:hover{color:#E0C0D8}
    body.dark .nav-tab.act{color:#F48CAC;border-bottom-color:#F48CAC}

    /* Calendar */
    body.dark .cal-day{color:#E0D0E8}
    body.dark .cal-day.ca:hover{background:#3A1E30;color:#F0A0C0}
    body.dark .cal-day.co{color:#504050}
    body.dark .cal-day.cfu{color:#504050}
    body.dark .ct:not(.cs){border-color:#F48CAC;color:#F48CAC}

    /* Time slots */
    body.dark .slot.sa{background:#2A1E30;color:#A8D8C0;border-color:#3A4840}
    body.dark .slot.sa:hover{background:#4A7060;color:#fff}
    body.dark .slot.st{background:#1E1520;color:#504050;border-color:#2A1E28}

    /* Toggle pills */
    body.dark .tog{background:#2A1E30}
    body.dark .tog-btn{color:#9080A0}
    body.dark .tog-btn.ton{background:#3A1E30;color:#F48CAC;box-shadow:0 2px 8px rgba(0,0,0,.3)}

    /* Service cards */
    body.dark .svc-c{background:#1E1520!important;border-color:#3A2840}
    body.dark .svc-c:hover{background:#2A1E30!important}
    body.dark .svc-sel{border-color:#C4627E!important}

    /* Badges */
    body.dark .badge{filter:brightness(.8) saturate(.9)}

    /* Bottom bars / footers */
    body.dark [style*="background:var(--white)"]{background:#1E1520!important}
    body.dark [style*="background:"var(--white)""]{background:#1E1520!important}
    body.dark [style*="borderTop:"1px solid #F0EAE4""]{border-color:#3A2840!important}
    body.dark [style*="background:var(--cream)"]{background:#170F1A!important}
    body.dark [style*="background:"var(--cream)""]{background:#170F1A!important}
    body.dark [style*="background:"var(--cream2)""]{background:#1E1520!important}

    /* Scrollbar */
    body.dark ::-webkit-scrollbar-thumb{background:#4A2848}
    body.dark ::-webkit-scrollbar-track{background:#1E1520}

    /* Link colors */
    body.dark a{color:#F48CAC}

    /* Code blocks */
    body.dark code{background:#2A1E30;color:#F0A0C0}

    /* Rose-lt backgrounds in dark */
    body.dark [style*="var(--rose-lt)"]{background:#3A1E28!important}
    body.dark [style*="background:"#F8F4F0""]{background:#1A1018!important}
    body.dark [style*="background:"#FAF6F0""]{background:#170F1A!important}
    body.dark [style*="background:"#F3EDE4""]{background:#1E1520!important}

    /* Cards with specific inline backgrounds */
    body.dark [style*="background:"linear-gradient(135deg,#FDF4F6"]{background:#1E1520!important}
    body.dark [style*="background:"#F8F4F0""]{background:#1A1018!important}
    body.dark [style*="background:"#F0FBF6""]{background:#0F1A14!important}

    /* Pro calendar */
    body.dark [style*="background:"#D4F0E4""]{background:#0F3020!important}
    body.dark [style*="background:"#FFF0C8""]{background:#2A2010!important}
    body.dark [style*="background:"#F0C8C8""]{background:#2A1010!important}

    /* Stat cards keep their gradient — looks great in dark */
    /* Cream/white in booking flows */
    body.dark [style*="background:"var(--cream)""]{background:#170F1A!important}
    /* ─ Bigger calendar dots ─ */
    .dot{position:absolute;bottom:2px;left:50%;transform:translateX(-50%);width:7px;height:7px;border-radius:50%}
    /* ─ Booking animations ─ */
    ...
    .bounceIn{animation:bounceIn .6s cubic-bezier(.22,1,.36,1) both}
    .svc-c:active{transform:scale(0.96)!important}
    .slot:active{transform:scale(0.9)}
    .btn-r:active{transform:scale(0.97)!important}
  `}</style>
);
...
function CalendarPage({ db, services, cfg, blocked, onNext, onBack }) {
  ...
  return (
    <div className="page">
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"20px 14px 100px",width:"100%"}}>
...
function NamesPage({ booking, onNext, onBack }) {
  ...
  return (
    <div className="page">
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"28px 14px 110px",width:"100%"}}>
...
function NotesPage({ booking, db, cfg, onDone, onBack }) {
  ...
  if (done) return (
    <div className="page" style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",padding:20}}>
      <G/>
      <div className="card bounceIn" style={{maxWidth:340,padding:"38px 26px",textAlign:"center"}}>
...
function ClientPortal({ db, services, cfg, blocked, dark, onToggleDark, onAdmin }) {
  ...
  return (
    <div className="page">
      <G/>
      {/* Hero */}
      <div style={{background: cfg?.heroBg ? `linear-gradient(rgba(0,0,0,.55),rgba(0,0,0,.55)), url(${cfg.heroBg}) center/cover no-repeat` : "linear-gradient(160deg,#2C1A1A 0%,#5A2535 55%,#8A3050 100%)",padding:"56px 20px 72px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <button onClick={onToggleDark} style={{position:"absolute",top:16,right:16,background:"rgba(255,255,255,.12)",border:"none",borderRadius:10,padding:"7px 11px",cursor:"pointer",fontSize:18,backdropFilter:"blur(8px)",transition:"all .2s"}} title={dark?"Modo claro":"Modo oscuro"}>
          {dark?"☀️":"🌙"}
        </button>
      </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
          {activeServices.map((s,i)=>(
            <button key={s.id} className={`card card-hover fu d${Math.min(i+1,5)}`}
              ...
              <div style={{width:"100%",height:70,borderRadius:10,overflow:"hidden",marginBottom:8,background:s.color||"#F8C8D4",display:"flex",alignItems:"center",justifyContent:"center"}}>
                {s.image
                  ? <img src={s.image} alt={s.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
  ...
  return (
    <div className="card" style={{padding:"20px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,color:"var(--text)"}}>📅 Calendario</h3>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={prevMo} style={{border:"none",background:"var(--rose-lt)",borderRadius:8,padding:"5px 8px",cursor:"pointer",color:"var(--rose)"}}><ChevronLeft size={14}/></button>
          <span style={{fontSize:13,fontWeight:600,color:"var(--text)",minWidth:100,textAlign:"center"}}>{MONTHS[month]} {year}</span>
          <button onClick={nextMo} style={{border:"none",background:"var(--rose-lt)",borderRadius:8,padding:"5px 8px",cursor:"pointer",color:"var(--rose)"}}><ChevronRight size={14}/></button>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:6}}>
        {["L","M","M","J","V","S","D"].map((l,i)=>(
          <div key={i} style={{textAlign:"center",fontSize:10,fontWeight:600,color:i===6?"#D4B4B4":"var(--text2)",padding:"2px 0"}}>{l}</div>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
        {Array.from({length:startCol}).map((_,i)=><div key={`e${i}`}/>)}
    ...
    <div style={{minHeight:"100vh",background:"var(--cream)"}}>
      ...
      <div style={{background:"var(--white)",borderBottom:"1px solid #F0EAE4",padding:"13px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:40}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={onToggleDark} style={{background:"none",border:"1.5px solid #E8DDD8",borderRadius:10,padding:"7px 11px",cursor:"pointer",fontSize:16,transition:"all .2s"}} title={dark?"Modo claro":"Modo oscuro"}>
            {dark?"☀️":"🌙"}
          </button>
        ...
        {view==="dashboard" && (
          <div className="fu">
            <div style={{marginBottom:22}}>
              <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,color:"var(--text)",marginBottom:2}}>Buenos días 💅</h2>
              <p style={{fontSize:13,color:"var(--text2)"}}>{new Date().toLocaleDateString("es-MX",{weekday:"long",day:"numeric",month:"long"})}</p>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              <div style={{background:"linear-gradient(135deg,#C4627E,#9E3F5A)",borderRadius:20,padding:"20px 18px",color:"#fff",boxShadow:"0 8px 24px rgba(196,98,126,.35)"}}>
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:700,lineHeight:1}}>{todayAps.length}</div>
              ...
              <div style={{background:"linear-gradient(135deg,#C9956A,#A07040)",borderRadius:20,padding:"20px 18px",color:"#fff",boxShadow:"0 8px 24px rgba(201,149,106,.35)"}}>
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:700,lineHeight:1}}>
              ...
              <div style={{background:"linear-gradient(135deg,#6C4E8A,#4A2868)",borderRadius:20,padding:"20px 18px",color:"#fff",boxShadow:"0 8px 24px rgba(108,78,138,.35)"}}>
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:700,lineHeight:1}}>{apts.length}</div>
              ...
              <button onClick={()=>setView("clients")} style={{background:"linear-gradient(135deg,#7A9E8E,#4A7060)",borderRadius:20,padding:"20px 18px",color:"#fff",boxShadow:"0 8px 24px rgba(122,158,142,.35)",border:"none",cursor:"pointer",textAlign:"left",transition:"all .2s"}}
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:700,lineHeight:1,color:"#fff"}}>{clients.length}</div>
...
function ConfigPanel({ cfg, onSave, onToast }) {
  const [f, setF] = useState(cfg || {salonName:"",salonLocation:"",slotInterval:30,heroTitle:"",heroSubtitle:"",heroBg:""});
  useEffect(()=>{ if(cfg) setF({slotInterval:30,heroTitle:"",heroSubtitle:"",heroBg:"",...cfg}); },[cfg]);
    ...
    <div className="fu">
      ...
      <div className="card" style={{padding:"24px",marginBottom:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div>
            <label style={{fontSize:12,fontWeight:600,color:"var(--text)",display:"block",marginBottom:5}}>URL de foto de fondo del banner</label>
            <div style={{display:"flex",gap:8}}>
              <input value={f.heroBg||""} onChange={e=>setF({...f,heroBg:e.target.value})} placeholder="https://i.imgur.com/tu-foto.jpg" style={{flex:1}}/>
              {f.heroBg && <button onClick={()=>setBgPreview(!bgPreview)} style={{background:"var(--rose-lt)",border:"none",borderRadius:10,padding:"10px 12px",cursor:"pointer",color:"var(--rose)",fontSize:12,fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>
                {bgPreview?"Ocultar":"Ver"}
              </button>}
            </div>
            <p style={{fontSize:11,color:"var(--text2)",marginTop:3}}>
              Puedes subir una foto a <a href="https://imgur.com/upload" target="_blank" rel="noreferrer" style={{color:"var(--rose)"}}>imgur.com</a> y pegar el link directo aquí (termina en .jpg o .png)
            </p>
          </div>
          {bgPreview && f.heroBg && (
            <div style={{borderRadius:14,overflow:"hidden",height:140,background:`linear-gradient(rgba(0,0,0,.5),rgba(0,0,0,.5)), url(${f.heroBg}) center/cover no-repeat`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{color:"#fff",fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:600}}>{f.heroTitle||f.salonName||"Nail Studio"}</span>
            </div>
          )}
        </div>
      </div>
