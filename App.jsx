  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--cream)"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"20px 14px 40px"}}>
        ...
        {view==="today" && (
          <div className="fu">
            ...
            {todayAps.length===0
              ...
              : todayAps.map(apt=><AptCard key={apt._id} apt={apt} onDel={()=>del(apt._id)} onConf={()=>conf(apt._id,apt.confirmed)} onWa={()=>waMsg(apt)}/>)
            }
          </div>
        )}
        ...
        {view==="upcoming" && (
          <div className="fu">
            ...
            {[...apts].sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time)).map(apt=>(
              <AptCard key={apt._id} apt={apt} onDel={()=>del(apt._id)} onConf={()=>conf(apt._id,apt.confirmed)} onWa={()=>waMsg(apt)}/>
            ))}
        ...
        {view==="clients" && (
          <div className="fu">
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:"var(--text)",marginBottom:18}}>Clientas ({clients.length})</h2>
            {clients.length===0
              ? <div className="card" style={{padding:"32px",textAlign:"center",color:"var(--text2)",fontSize:14}}>Cuando alguien agende aparecerá aquí 🌸</div>
              : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:12}}>
                {clients.map(c=>{
                  ...
                  return (
                    <div key={c._id} className="card card-hover" style={{padding:"18px"}}>
...
function AptCard({ apt, onDel, onConf, onWa }) {
  return (
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0}}>
          <button onClick={onConf}
            ...
            {apt.confirmed?"✓":"Confirmar"}
          </button>
          <button className="btn-r" onClick={onWa} style={{borderRadius:8,padding:"7px 10px",fontSize:11,display:"flex",alignItems:"center",gap:3}}>
...
function AddAppointmentForm({ db, services, onSave }) {
  ...
  const save = async () => {
    ...
    await addDoc(collection(db,"appointments"),{
      ...
      notes:"",companion:null,createdAt:serverTimestamp(),
    });
    // upsert client
    const snap = await getDocs(collection(db,"clients"));
