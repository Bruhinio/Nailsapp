  ...
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      ...
      --rose:#FF2D55;--rose-dk:#CC1A3E;--rose-lt:rgba(255,45,85,.1);
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--ios-bg)",paddingBottom:83}}>
      ...
      <div className="ios-topbar" style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {newApts > 0 && (
            <button onClick={()=>{setNew(0);setView("dashboard");}}
        ...
        {view==="dashboard" && (
          <div className="fu">
            ...
            <div style={{display:"flex",gap:10,overflowX:"auto",padding:"16px 0 4px",marginLeft:-14,paddingLeft:14,marginRight:-14,paddingRight:14}}>
              {[
                {label:"Hoy", value:todayAps.length, sub:`$${todayAps.reduce((s,a)=>s+(a.price||0),0)} estim.`, bg:"linear-gradient(135deg,#FF2D55,#C4184A)", sh:"0 8px 24px rgba(255,45,85,.35)"},
            ...
            <div className="card fu" style={{padding:"14px 16px",marginTop:14,marginBottom:14,borderRadius:18}}>
              ...
              <div style={{display:"flex",gap:3,alignItems:"flex-end",height:44}}>
                {["40%","55%","48%","72%","68%","88%","100%"].map((h,i)=>(
                  <div key={i} style={{flex:1,background:i===6?"var(--ios-tint)":`rgba(255,45,85,${0.2+i*0.1})`,borderRadius:"3px 3px 0 0",height:h,transition:"height .4s"}}/>
                ))}
              </div>
          ...
          return (
            <div className="fu">
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                background:"var(--ios-card)",borderRadius:14,padding:"10px 14px",
                marginBottom:0,boxShadow:"var(--ios-sh)"}}>
                      ...
                      {apt ? (
                        <div style={{background:"var(--ios-card)",borderRadius:11,
                          ...
                          <div style={{display:"flex",gap:6,marginTop:8}}>
                            <button onClick={()=>conf(apt._id,apt.confirmed)}
                              style={{fontSize:10,fontWeight:600,padding:"4px 9px",borderRadius:7,border:"none",cursor:"pointer",
                                background:apt.confirmed?"rgba(255,45,85,.1)":"rgba(52,199,89,.12)",
                    ...
                    <div key={ds} style={{marginBottom:20}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                        <div style={{fontSize:12,fontWeight:700,color:"var(--ios-tint)",
                          background:"rgba(255,45,85,.1)",borderRadius:8,padding:"3px 10px",flexShrink:0}}>
                      ...
                      {upcomingByDay[ds].map(apt=>(
                        <AptCard key={apt._id} apt={apt} db={db} services={services}
                          onDel={()=>del(apt._id)} onConf={()=>conf(apt._id,apt.confirmed)}
